...
function GameCard({ game, onClick }) {
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop: game.live ? 8 : 0 }}>
        <div style={{ textAlign:'center', flex:1 }}>
          <div style={{ fontSize:18, fontWeight:900, color:'#e2e8f0' }}>{game.away}</div>
          {game.awayScore != null && <div style={{ fontSize:26, fontWeight:900, color:'#e2e8f0' }}>{game.awayScore}</div>}
        </div>
        <div style={{ textAlign:'center', padding:'0 12px' }}>
          {game.live && <span style={{ fontSize:10, color:'#ef4444', border:'1px solid #ef444444', borderRadius:4, padding:'2px 6px', fontWeight:700 }}>LIVE</span>}
          {game.upcoming && <span style={{ fontSize:10, color:'#3b82f6', border:'1px solid #3b82f644', borderRadius:4, padding:'2px 6px', fontWeight:700 }}>예정</span>}
          {game.done && <span style={{ fontSize:10, color:'#64748b', border:'1px solid #64748b44', borderRadius:4, padding:'2px 6px' }}>종료</span>}
          <div style={{ fontSize:11, color:'#64748b', marginTop:5 }}>{game.inning}</div>
          {game.live && <div style={{ fontSize:10, color:'#3b82f6', marginTop:3 }}>탭하여 참여</div>}
        </div>
        <div style={{ textAlign:'center', flex:1 }}>
          <div style={{ fontSize:18, fontWeight:900, color:'#e2e8f0' }}>{game.home}</div>
          {game.homeScore != null && <div style={{ fontSize:26, fontWeight:900, color:'#e2e8f0' }}>{game.homeScore}</div>}
        </div>
      </div>
    </div>
  );
